          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link key={tab.name} href={tab.path}>
                <button className={`pb-4 px-1 flex flex-col items-start relative transition-all ${isActive ? "text-[#153a30]" : "text-gray-300 hover:text-gray-500"}`}>
                  <span className="text-xs font-bold mb-1">{tab.name === "Interviews" ? interviews.length : 0}</span>
                  <span className="text-lg font-bold">{tab.name}</span>
                  {isActive && <div className="absolute bottom-0 left-0 w-full h-[3.5px] bg-[#153a30] rounded-full" />}
                </button>
              </Link>
            );
          })}
