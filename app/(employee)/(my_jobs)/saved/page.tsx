    ...
    const fetchSaved = async () => {
      try {
        ...
        const mapped = (data || []).map((item: any) => ({
          ...
          salary: item.salary,
          tags: [item.type, item.arrangement, item.experience].filter(Boolean),
        }));
      ...
      <main className="max-w-5xl mx-auto p-12">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link key={tab.name} href={tab.path}>
                ...
                >
                  <span className="text-xs font-bold mb-1">{tab.name === "Saved" ? savedJobs.length : 0}</span>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {savedJobs.map((job) => (
              <div key={job.id} className="py-6 border-b border-gray-400 group">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex items-center gap-3">
                    <Link href={`/jobs/${job.id}/apply`}>
                      <button className="bg-[#153a30] text-white px-6 py-1.5 rounded-lg font-bold text-sm hover:bg-[#0d2a23] transition-colors">
                        Apply
                      </button>
                    </Link>
